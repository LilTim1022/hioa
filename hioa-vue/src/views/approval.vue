	...
	<div v-if="isAuth(['ROOT', 'WORKFLOW:APPROVAL', 'FILE:ARCHIVE'])">
		<el-form :inline="true" :model="dataForm" :rules="dataRule" ref="dataForm" @keyup.enter.native="searchHandle()">
			<el-form-item prop="creatorName">
				<el-input v-model="dataForm.creatorName" size="medium" placeholder="Creator Name" clearable="clearable" />
			</el-form-item>
			...
			<el-form-item prop="instanceId">
				<el-input v-model="dataForm.instanceId" size="medium" placeholder="InstanceId" clearable="clearable" />
			</el-form-item>
			<el-form-item>
      ...
      if(expandedRows.length>0) {
        ...
        if(row) {
          ...
          that.$http("approval/searchApprovalContent","POST",data,false,function(resp){
            ...
            if(content.hasOwnProperty("files")) { //如果有files属性，说明工作流绑定了归档文件
                for(let one of content.files) { //从数组取出元素，是json格式的数据
                  //对模型层赋值
        ...
        //如果表达式成立证明想展开面板，else语句则是表达式不成立，想要收缩所有面板
        that.expands = [];
      }
    },
