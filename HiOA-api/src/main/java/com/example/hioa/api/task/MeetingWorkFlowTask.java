...
public class MeetingWorkFlowTask {
    ...
    @org.springframework.beans.factory.annotation.Value("${hioa.recieveNotify}")
    private String recieveNotify;


    @org.springframework.beans.factory.annotation.Value("${workflow.url}")
    ...
    public void startMeetingWorkflow(String uuid, int creatorId,String title, String date, String start,String meetingType) {
        ...
        if (resp.getStatus() == 200) {
            ...
            if (row != 1) {
                throw new HioaException("保存会议工作流实例ID失败");
            }
        } else {
            log.error(resp.body());
        }
